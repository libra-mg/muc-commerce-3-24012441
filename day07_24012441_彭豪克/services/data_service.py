...
def load_dashboard_data(base_dir: Path, selected_category: str = "全部") -> dict:
    ...
    max_stage = max_row["TenureGroup"]
    max_rate = max_row["流失率"]
    insight = (
        f"生命周期阶段 {max_stage} 的流失率最高，达到{max_rate:.1%},"
        f"该阶段共有{int(max_row['用户数']):,}名用户。"
        "注意：该结论基于截面数据，不能直接用于推断因果，"
        "且 36+ 月用户样本仅 4 人，统计可靠性较低。"
    )
